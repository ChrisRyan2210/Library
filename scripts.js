...
class Library {
    ...
    removeBook(bookID) {
        ...
        const index = this.library.findIndex(book => book.bookID === bookID);
        if (index !== -1) {
            this.library.splice(index, 1); // Remove the book from the array
        }
    };
    ...
    changeReadStatus(bookID) {
        ...
        if (book) {
            ...
            book.toggleRead(); // If true, set to false; if false, set to true  
    ...
    addBook(book) {
        ...
        bookIDtd.textContent = book.bookID;
...
//setting up dummy data to show on initial load
const book = new Book(crypto.randomUUID(), "Rowling", "Harry Potter", 255, true); 
const secondBook = new Book(crypto.randomUUID(), "Tolkien", "The Hobbit", 377, true); 
...
myLibrary.getBooks().forEach((book) => {
    ...
    btnRead.addEventListener("click", () => {
        const bookID = readtd.dataset.id;//grabbing the html dataset id attribute 
        myLibrary.changeReadStatus(bookID);
    });
    btnReadtd.appendChild(btnRead);
